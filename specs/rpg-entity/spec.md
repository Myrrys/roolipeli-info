...
| `based_on_game_id` | UUID (FK → games) | nullable, self-referential | `isBasedOn` |
...
- `games` → `games` (self-ref via `based_on_game_id`) — "isBasedOn" (e.g., Spore Borg → Mork Borg)
...
- Same policy pattern for `games_creators`, `game_semantic_labels`, `game_references`
...
2. **`/pelit/[slug]`** (Game Detail)
   ...
   - "Based on" link if `based_on_game_id` is set
...
5. **`/admin/games/[id]/edit`** (Edit Game)
   ...
   - "Based on" dropdown (other games, excluding self)
...
```json
{
  ...
  "isBasedOn": {
    "@type": "Game",
    "name": "...",
    "url": "https://roolipeli.info/pelit/..."
  },
  "hasPart": [
...
#### Database & Types
- [ ] Migration creates `games`, `games_creators`, `game_semantic_labels`, `game_references` tables
- [ ] Migration adds `game_id` FK to `products` table
- [ ] RLS policies: public SELECT, admin-only INSERT/UPDATE/DELETE on all new tables
- [ ] Zod schemas added to `@roolipeli/database` (`GameSchema`, `GameCreatorSchema`, `GameSemanticLabelSchema`, `GameReferenceSchema`)
...
**Scenario: View game based on another game**
- Given: Game "Spore Borg" exists with `based_on_game_id` pointing to "Mork Borg"
- When: I visit `/pelit/spore-borg`
- Then: I see "Based on: Mork Borg" with a link to `/pelit/mork-borg`
